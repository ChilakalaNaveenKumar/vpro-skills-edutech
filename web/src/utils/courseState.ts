import { localHourRange } from './hours'
import type { ScheduleRow } from './schedule'

export type CourseState = 'In session' | 'Starting soon' | 'Gathering interest'

/** One running batch, in the three shapes the UI needs it in. */
export interface CourseBatch {
  /** "Batch A-04" */
  number: string
  /** "Weekdays", or empty when the batch does not say. */
  days: string
  /** "10:00 – 11:30 AM EDT" in the visitor's timezone */
  hour: string
  /** "Weekdays 10:00 – 11:30 AM EDT", or just the hour when days are unset. */
  daysHour: string
  /** "started 6 Aug" - or "starts 27 Sep" if it has not begun. */
  when: string
  /** "Batch A-04, started 6 Aug" - or "starts 27 Sep" if it has not begun. */
  note: string
  /** Everything a WhatsApp reply needs: number, hour and both dates. */
  line: string
  /** False for a batch with a start date still ahead of it. */
  started: boolean
}

// A batch that is teaching, mid-run, or starting later today all mean the same
// thing to a visitor: this course is running.
const RUNNING_STATES: ReadonlySet<ScheduleRow['state']> = new Set(['live', 'today', 'running'])

// A course can have more than one batch teaching at once - a morning cohort and
// an evening one is the normal case for a training centre. This used to be a
// `.find`, so the second cohort was silently invisible: its hour never appeared
// on the shelf and its number never reached the message a visitor sent us.
//
// 'upcoming' counts as scheduled. It used to be filtered out entirely, which
// meant a course with a dated batch opening in three weeks displayed "Hours: Not
// scheduled" and asked the visitor to register interest - contradicting the
// batch sitting on the schedule page with its date printed on it. A batch that
// has not started yet is still a batch someone can take a seat in; whether it
// has begun changes the wording, not whether it exists.
function scheduledRows(courseName: string, rows: ScheduleRow[] | null): ScheduleRow[] {
  if (!rows) return []
  return rows.filter(
    (row) => row.batch.course_name === courseName && row.batch.progress_status !== 'COMPLETED',
  )
}

function isRunning(row: ScheduleRow): boolean {
  return RUNNING_STATES.has(row.state)
}

function shortDate(iso: string): string {
  return new Date(`${iso}T00:00:00`).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })
}

/** Every batch of this course that is teaching or about to.
 *
 * Batches that have not begun come first, matching the schedule page: a cohort
 * you can join from its first topic is the better offer, and one already three
 * weeks in is the fallback. */
export function courseBatches(courseName: string, rows: ScheduleRow[] | null): CourseBatch[] {
  return scheduledRows(courseName, rows)
    .slice()
    .sort((a, b) => Number(isRunning(a)) - Number(isRunning(b)))
    .map((row) => {
      // The comp says "Weekdays 7:30 - 9:00 PM"; `Batch` carries no day-of-week
      // field, so the window is rendered without inventing which days it falls
      // on.
      const hour = localHourRange(row.batch.start_time, row.batch.end_time, row.batch.start_date)
      const days = row.batch.days_of_week ?? ''
      const started = isRunning(row)
      const when = `${started ? 'started' : 'starts'} ${shortDate(row.batch.start_date)}`
      return {
        number: row.batch.batch_number,
        days,
        hour,
        daysHour: days ? `${days} ${hour}` : hour,
        when,
        note: `${row.batch.batch_number}, ${when}`,
        line: `${row.batch.batch_number}, ${hour}, ${shortDate(row.batch.start_date)} to ${shortDate(row.batch.end_date)}`,
        started,
      }
    })
}

// Derived, never stored. A `state` column on courses.ts would drift the moment
// a batch ended and nobody remembered to edit it.
export function courseStateFor(courseName: string, rows: ScheduleRow[] | null): CourseState {
  const scheduled = scheduledRows(courseName, rows)
  if (scheduled.some(isRunning)) return 'In session'
  return scheduled.length > 0 ? 'Starting soon' : 'Gathering interest'
}
